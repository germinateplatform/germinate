...
package jhi.germinate.client.page.phenotype;
 ...
 */
public class PhenotypePage extends Composite implements HasHyperlinkButton, HasLibraries
{
	interface PhenotypePageUiBinder extends UiBinder<HTMLPanel, PhenotypePage>
	{
	}

	private static PhenotypePageUiBinder ourUiBinder = GWT.create(PhenotypePageUiBinder.class);
	...
	public PhenotypePage()
	{
		...
		exportSelection = new DataExportSelection<>(ExperimentType.phenotype);
	...
	protected void onLoad()
	{
		...
		selectedDatasets = LongListParameterStore.Inst.get().get(Parameter.phenotypeDatasetIds);
	...
	private void getPhenotypes()
	{
		final ParallelAsyncCallback<ServerResult<List<Phenotype>>> phenotypeCallback = new ParallelAsyncCallback<ServerResult<List<Phenotype>>>()
		{
			...
			{
				PhenotypeService.Inst.get().get(Cookie.getRequestProperties(), selectedDatasets, ExperimentType.phenotype, false, this);
			}
		};
		final ParallelAsyncCallback<ServerResult<List<Group>>> groupCallback = new ParallelAsyncCallback<ServerResult<List<Group>>>()
		{
			...
			protected void start()
			{
				GroupService.Inst.get().getAccessionGroups(Cookie.getRequestProperties(), selectedDatasets, ExperimentType.phenotype, this);
			}
		};
		...
		new ParallelParentAsyncCallback(true, phenotypeCallback, groupCallback)
		{
			...
			public void handleSuccess()
			{
				...
				phenotypeByPhenotypeChart.update(ExperimentType.phenotype, getNumericalPhenotypes(), groups, Text.LANG.trialsPByPText());
				matrixChart.update(ExperimentType.phenotype, getNumericalPhenotypes(), groups);
	...
	public HyperlinkPopupOptions getHyperlinkOptions()
	{
		return new HyperlinkPopupOptions()
				.setPage(Page.CATEGORICAL_EXPORT)
				.addParam(Parameter.phenotypeDatasetIds);
	}
