...
public abstract class AbstractCartView<T extends DatabaseObject> extends GerminateComposite
{
	...
	protected abstract ShoppingCart.ItemType getItemType();
	...
	protected void setUpContent()
	{
		...
		else
		{
			...
			{
				ShoppingCart.clear(getItemType());
			...
			FileDownloadWidget widget = new OnDemandFileDownloadWidget((index, callback) -> writeToFile(new ArrayList<>(ShoppingCart.get(getItemType())), callback), true)
	...
	public static void askForGroupNameAndCreate(List<String> markedIds, ShoppingCart.ItemType itemType, AsyncCallback<ServerResult<Long>> callback)
	{
		GroupService.Inst.get().getTypes(Cookie.getRequestProperties(), new DefaultAsyncCallback<ServerResult<List<GroupType>>>()
		{
			@Override
	 ...
	 */
	private static boolean addNewGroup(String newGroup, final List<String> newGroupMembers, ShoppingCart.ItemType type, AsyncCallback<ServerResult<Long>> callback)
	{
		...
		GroupService.Inst.get().createNew(Cookie.getRequestProperties(), strippedString, type.getTarget(), callback != null ? callback : new DefaultAsyncCallback<ServerResult<Long>>()
		{
			@Override
			public void onSuccessImpl(ServerResult<Long> result)
			{
				...
				if (!CollectionUtils.isEmpty(newGroupMembers))
					addGroupMembers(newGroupMembers, result.getServerResult(), type);
			}
		});
	...
	private static void addGroupMembers(List<String> newGroupMembers, Long groupId, ShoppingCart.ItemType type)
	{
