 ...
 */
public class PDCIStats extends Composite
{
	...
	protected void onLoad()
	{
		...
		AccessionService.Inst.get().getPDCIStats(Cookie.getRequestProperties(), new DefaultAsyncCallback<ServerResult<String>>(true)
		{
			...
			protected void onFailureImpl(Throwable caught)
			{
				...
				PDCIStats.this.removeFromParent();
			}
			...
			protected void onSuccessImpl(ServerResult<String> result)
			{
				if (result.hasData())
				{
					...
					PlotlyBarChart.Config config = new PlotlyBarChart.Config()
							.setxAxisTitle(Text.LANG.passportColumnPDCI())
							.setyAxisTitle(Text.LANG.generalCount())
							.setX("bin")
							.setFilePath(filePath)
							.setDownloadFilename("pdci-distribution")
