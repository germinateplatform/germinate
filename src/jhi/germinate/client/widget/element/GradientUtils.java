...
public class GradientUtils
{
	...
	public static Widget createHorizontalGradientLegend(Gradient gradient, boolean showActualValue)
	{
		return createHorizontalGradientLegend(gradient, HorizontalLegendPosition.BOTTOM, showActualValue);
	}

	/**
	 * Creates a vertical gradient legend (running from min to max)
	 *
	 * @param overallHeight The overall height of the widget
	 * @param gradient      The {@link Gradient} to use
	 * @return The created gradient legend
	 */
	public static Widget createVerticalGradientLegend(int overallHeight, Gradient gradient)
	{
		FlowPanel overall = new FlowPanel();
		overall.getElement().getStyle().setTextAlign(TextAlign.CENTER);

		Label high = new Label(Text.LANG.generalHigh());
		overall.add(high);
		...
		if (Canvas.isSupported())
		{
			...
			canvas.setWidth(DEFAULT_SIZE + "px");
			canvas.setHeight(overallHeight + "px");
			canvas.setCoordinateSpaceWidth(DEFAULT_SIZE);
			canvas.setCoordinateSpaceHeight(overallHeight);
			canvas.getElement().getStyle().setVerticalAlign(VerticalAlign.MIDDLE);

			Context2d context = canvas.getContext2d();

			CanvasGradient cGradient = context.createLinearGradient(0, 5, 0, overallHeight - 5);

            /* Add an intermediate step for each color */
			int size = gradient.getColors().length;
			for (int i = 0; i < size; i++)
			{
				cGradient.addColorStop((1.0f * i) / (size - 1), gradient.getColors()[size - i - 1].toHexValue());
			}

			context.setFillStyle(cGradient);
			context.fillRect(0, 0, DEFAULT_SIZE, overallHeight);

			overall.add(canvas);
		}
		else
		{
			int height = overallHeight / gradient.getColors().length;
			/* Add a div for each of the colors */
			for (int i = gradient.getColors().length - 1; i >= 0; i--)
			{
				SafeStyles background = SafeStylesUtils.forTrustedBackgroundColor(gradient.getColors()[i].toRGBValue());
				SafeStyles lineHeight = SafeStylesUtils.forLineHeight(height, Unit.PX);
				SafeStyles cellWidth = SafeStylesUtils.forWidth(DEFAULT_SIZE, Unit.PX);
				HTML element = new HTML(SimpleHtmlTemplate.INSTANCE.gradientLegendVertical(background, lineHeight, cellWidth));
				overall.add(element);
			}
		}

		Label low = new Label(Text.LANG.generalLow());
		overall.add(low);
