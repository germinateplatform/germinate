/*
 *  Copyright 2017 Sebastian Raubach and Paul Shaw from the
 *  Information and Computational Sciences Group at JHI Dundee
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 */

package jhi.germinate.client.widget.table.pagination;

import com.google.gwt.cell.client.*;
import com.google.gwt.dom.client.*;
import com.google.gwt.user.cellview.client.*;
import com.google.gwt.user.client.rpc.*;

import jhi.germinate.client.i18n.Text;
import jhi.germinate.client.util.*;
import jhi.germinate.shared.Style;
import jhi.germinate.shared.datastructure.*;
import jhi.germinate.shared.datastructure.database.*;
import jhi.germinate.shared.search.*;

/**
 * @author Sebastian Raubach
 */
public abstract class GenotypeTable extends DatabaseObjectPaginationTable<Genotype>
{
	public GenotypeTable(SelectionMode selectionMode, boolean sortingEnabled)
	{
		super(selectionMode, sortingEnabled);
	}

	@Override
	protected boolean supportsFiltering()
	{
		return false;
	}

	@Override
	protected boolean supportsDownload()
	{
		return false;
	}

	@Override
	protected void download(PartialSearchQuery filter, AsyncCallback<ServerResult<String>> callback)
	{
		callback.onSuccess(null);
	}

	@Override
	protected void createColumns()
	{
		Column<Genotype, ?> column;

		if (!GerminateSettingsHolder.get().hideIdColumn.getValue())
		{
			column = new TextColumn()
			{
				@Override
				public String getValue(Genotype object)
				{
					return Long.toString(object.getId());
				}

				@Override
				public Class getType()
				{
					return Long.class;
				}

				@Override
				public String getCellStyleNames(Cell.Context context, Genotype object)
				{
					return Style.LAYOUT_WHITE_SPACE_NO_WRAP;
				}
			};
			column.setDataStoreName(Genotype.ID);
			addColumn(column, Text.LANG.accessionsColumnId(), true);
		}

		/* Add the accession gid column */
		column = new TextColumn()
		{
			@Override
			public String getValue(Genotype object)
			{
				if (object.getAccession() != null)
					return object.getAccession().getGeneralIdentifier();
				else
					return null;
			}

			@Override
			public Class getType()
			{
				return Long.class;
			}
		};
		column.setDataStoreName(Accession.GENERAL_IDENTIFIER);
		addColumn(column, Text.LANG.accessionsColumnGeneralIdentifier(), true);

		/* Add the accession name column */
		column = new TextColumn()
		{
			@Override
			public String getValue(Genotype object)
			{
				if (object.getAccession() != null)
					return object.getAccession().getName();
				else
					return null;
			}

			@Override
			public Class getType()
			{
				return String.class;
			}
		};
		column.setDataStoreName(Accession.NAME);
		addColumn(column, Text.LANG.accessionsColumnName(), true);

		/* Add the accession number column */
		column = new TextColumn()
		{
			@Override
			public String getValue(Genotype object)
			{
				if (object.getAccession() != null)
					return object.getAccession().getNumber();
				else
					return null;
			}

			@Override
			public Class getType()
			{
				return String.class;
			}
		};
		column.setDataStoreName(Accession.NUMBER);
		addColumn(column, Text.LANG.accessionsColumnNumber(), true);

		/* Add the allele one column */
		column = new TextColumn()
		{
			@Override
			public String getValue(Genotype object)
			{
				return object.getAlleleOne();
			}

			@Override
			public Class getType()
			{
				return String.class;
			}
		};
		column.setDataStoreName(Genotype.ALLELE_1);
		addColumn(column, Text.LANG.markersColumnAlleleOne(), true);

		/* Add the allele two column */
		column = new TextColumn()
		{
			@Override
			public String getValue(Genotype object)
			{
				return object.getAlleleTwo();
			}

			@Override
			public Class getType()
			{
				return String.class;
			}
		};
		column.setDataStoreName(Genotype.ALLELE_2);
		addColumn(column, Text.LANG.markersColumnAlleleTwo(), true);
	}

	@Override
	protected void onSelectionChanged(NativeEvent event, Genotype object, int column)
	{
	}
}
