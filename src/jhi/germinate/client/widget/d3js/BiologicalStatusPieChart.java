 ...
 */
public class BiologicalStatusPieChart extends AbstractChart
{
	@Override
		...
		CommonService.Inst.get().getBiologicalStatusStats(Cookie.getRequestProperties(), new DefaultAsyncCallback<ServerResult<String>>(true)
		{
			...
			protected void onSuccessImpl(ServerResult<String> result)
			{
				if (result.hasData())
				{
					...
					BiologicalStatusPieChart.this.onResize(true);
				}
				else
				{
	...
	{
		return "biological-status-stats";
	}
	...
	private void onClickSegment(String biologicalstatus)
	{
		try
		{
			PartialSearchQuery query = new PartialSearchQuery();
			query.add(new SearchCondition(BiologicalStatus.SAMPSTAT, new Like(), biologicalstatus, String.class));
			...
			new $wnd.d3pie(panelId, {
				...
				callbacks: {
					onClickSegment: function (a) {
						that.@jhi.germinate.client.widget.d3js.BiologicalStatusPieChart::onClickSegment(*)(a.data.datum.biologicalstatus);
					}
				}
			});
		});
