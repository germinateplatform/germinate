...
public class ViewInitializer extends DatabaseInitializer
{
	...
	private static final String QUERY_DATA_OVERVIEW           = "SELECT ( SELECT count(1) FROM germinatebase ) AS accessions, ( SELECT count(1) FROM markers ) AS markers, ( SELECT count(1) FROM locations LEFT JOIN locationtypes ON locations.locationtype_id = locationtypes.id WHERE ( locationtypes.name = 'collectingsites' ) ) AS collectingsites, ( SELECT count(1) FROM genotypes ) AS genotypes, ( SELECT count(1) FROM phenotypedata LEFT JOIN datasets ON datasets.id = phenotypedata.dataset_id LEFT JOIN experiments ON experiments.id = datasets.experiment_id LEFT JOIN experimenttypes ON experimenttypes.id = experiments.experiment_type_id WHERE experimenttypes.description = 'phenotype' ) AS phenotypes";
	...
	private static final String[] COLUMNS_DATA_OVERVIEW           = {"accessions", "markers", "collectingsites", "genotypes", "phenotypes"};
