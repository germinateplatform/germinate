...
{
	public static final  String COMMON_SYNONYMS               = " LEFT JOIN `synonyms` ON (`synonyms`.`foreign_id` = `phenotypes`.`id` AND `synonyms`.`synonymtype_id` = " + SynonymType.phenotypes.getId() + " )";
	...
	private static final String[] COLUMNS_SORTABLE = {Phenotype.ID, Phenotype.NAME, Phenotype.DESCRIPTION, Phenotype.CREATED_ON, Phenotype.UPDATED_ON, Unit.NAME, Unit.ABBREVIATION, Unit.DESCRIPTION, Phenotype.COUNT, Synonym.SYNONYM};
	private static final String COMMON_TABLES                 = " `germinatebase` LEFT JOIN `entitytypes` ON `entitytypes`.`id` = `germinatebase`.`entitytype_id` LEFT JOIN `phenotypedata` ON `germinatebase`.`id` = `phenotypedata`.`germinatebase_id` LEFT JOIN `phenotypes` ON `phenotypes`.`id` = `phenotypedata`.`phenotype_id` LEFT JOIN `units` ON `units`.`id` = `phenotypes`.`unit_id` LEFT JOIN `datasets` ON `datasets`.`id` = `phenotypedata`.`dataset_id` LEFT JOIN `experiments` ON `experiments`.`id` = `datasets`.`experiment_id` LEFT JOIN `experimenttypes` ON `experimenttypes`.`id` = `experiments`.`experiment_type_id`";
	private static final String COMMON_TABLES_MIN             = " `phenotypes` LEFT JOIN `units` ON `units`.`id` = `phenotypes`.`unit_id` ";
	...
	private static final String SELECT_DATA_FOR_FILTER_EXPORT = "SELECT `germinatebase`.`id` AS germinatebase_id, `germinatebase`.`name` AS germinatebase_name, `germinatebase`.`general_identifier` AS germinatebase_gid, `datasets`.`name` AS datasets_name, `datasets`.`description` AS datasets_description, `experimenttypes`.`description` AS experimenttypes_description, `locations`.`site_name` AS locations_site_name, `countries`.`country_name` AS countries_name, `phenotypes`.`name` AS phenotypes_name, `phenotypes`.`short_name` AS phenotypes_short_name, `units`.`unit_name` AS units_name, `phenotypedata`.`phenotype_value` AS phenotypedata_value FROM " + COMMON_TABLES + "  LEFT JOIN `locations` ON `phenotypedata`.`location_id` = `locations`.`id` LEFT JOIN `countries` ON `countries`.`id` = `locations`.`country_id` {{FILTER}} AND `datasets`.`id` IN (%s) %s LIMIT ?, ?";
	private static final String SELECT_IDS_FOR_FILTER         = "SELECT DISTINCT `germinatebase`.`id` FROM " + COMMON_TABLES + " LEFT JOIN `locations` ON `phenotypedata`.`location_id` = `locations`.`id` LEFT JOIN `countries` ON `countries`.`id` = `locations`.`country_id` {{FILTER}} AND `datasets`.`id` IN (%s)";
	private static final String SELECT_ALL_FOR_FILTER         = "SELECT `phenotypes`.*, `units`.*, `synonyms`.`synonyms`, (SELECT COUNT(1) FROM `phenotypedata` WHERE `phenotypedata`.`phenotype_id` = `phenotypes`.`id`) AS count FROM " + COMMON_TABLES_MIN + " " + COMMON_SYNONYMS +  " {{FILTER}} GROUP BY `phenotypes`.`id`, `synonyms`.`id` %s LIMIT ?, ?";
	...
	public static ServerResult<List<String>> getIdsForFilter(UserAuth user, PartialSearchQuery filter) throws DatabaseException, InvalidSearchQueryException, InvalidArgumentException, InvalidColumnException
	{
		...
		return AbstractManager.getFilteredValueQuery(filter, user, formatted, PhenotypeService.COLUMNS_DATA_SORTABLE)
				.setLongs(datasetIds)
				.run(Accession.ID)
				.getStrings();
	}
