...
public class MarkerManager extends AbstractManager<Marker>
{
	...
	public static final  String SELECT_SYNONYMS             = " `mapdefinitions`.*, `mapfeaturetypes`.*, `maps`.*, `markers`.*, `synonyms`.`synonyms` AS synonyms";
	...
	private static final String SELECT_ALL_FOR_GROUP        = "SELECT `markers`.*, `markertypes`.*, `synonyms`.`synonyms` AS synonyms FROM `markers` LEFT JOIN `markertypes` ON `markertypes`.`id` = `markers`.`markertype_id` LEFT JOIN `groupmembers` ON `markers`.`id` = `groupmembers`.`foreign_id` LEFT JOIN `groups` ON `groups`.`id` = `groupmembers`.`group_id` " + COMMOM_SYNONYMS + " WHERE `groups`.`id` = ? %s LIMIT ?, ?";
	private static final String SELECT_IDS_FOR_GROUP        = "SELECT `markers`.`id` FROM `markers` LEFT JOIN `markertypes` ON `markertypes`.`id` = `markers`.`markertype_id` LEFT JOIN `groupmembers` ON `markers`.`id` = `groupmembers`.`foreign_id` LEFT JOIN `groups` ON `groups`.`id` = `groupmembers`.`group_id` WHERE `groups`.`id` = ?";
	...
	public static ServerResult<List<String>> getNamesForMap(UserAuth userAuth, Long mapId) throws DatabaseException
	{
		return new ValueQuery(SELECT_NAMES_FOR_MAP, userAuth)
				.setLong(mapId)
				.setLong(userAuth.getId())
				.run(Marker.MARKER_NAME)
				.getStrings();
