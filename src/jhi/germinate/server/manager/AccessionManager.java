...
public class AccessionManager extends AbstractManager<Accession>
{
	...
	private static final String SELECT_SYNONYMS = "germinatebase.*, entitytypes.*, subtaxa.*, taxonomies.*, locations.*, countries.*, biologicalstatus.*, institutions.*, collectingsources.*, GROUP_CONCAT(synonyms.synonym SEPARATOR ', ') AS synonyms";
	...
	private static final String SELECT_ALL_FOR_MEGA_ENV       = "SELECT " + SELECT_SYNONYMS + " FROM " + COMMON_TABLES + " " + COMMOM_SYNONYMS + " LEFT JOIN megaenvironmentdata ON megaenvironmentdata.location_id = locations.id LEFT JOIN megaenvironments ON megaenvironments.id = megaenvironmentdata.megaenvironment_id WHERE megaenvironments.id = ? GROUP BY germinatebase.id %s LIMIT ?, ?";
	private static final String SELECT_ALL_FOR_MEGA_ENV_UNK   = "SELECT " + SELECT_SYNONYMS + " FROM " + COMMON_TABLES + " " + COMMOM_SYNONYMS + " WHERE location_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM megaenvironmentdata WHERE megaenvironmentdata.location_id = locations.id) GROUP BY germinatebase.id %s LIMIT ?, ?";
	private static final String SELECT_FOR_GROUP              = "SELECT " + SELECT_SYNONYMS + " FROM " + COMMON_TABLES + " " + COMMOM_SYNONYMS + " LEFT JOIN groupmembers ON germinatebase.id = groupmembers.foreign_id LEFT JOIN groups ON groups.id = groupmembers.group_id WHERE groups.id = ? GROUP BY germinatebase.id %s LIMIT ?, ?";
