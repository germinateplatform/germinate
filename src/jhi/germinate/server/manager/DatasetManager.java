...
public class DatasetManager extends AbstractManager<Dataset>
{
	...
	private static final String SELECT_ALL_FOR_TRAIT       = "SELECT `datasets`.*,  `experiments`.*, `experimenttypes`.*, `locations`.*, `countries`.*, `licenses`.*, COUNT(DISTINCT `germinatebase_id`) AS nr_of_data_objects, COUNT(`phenotype_value`) AS nr_of_data_points FROM " + DATA_POINTS_SUB_QUERY + " LEFT JOIN `phenotypedata` ON `phenotypedata`.`dataset_id` = `datasets`.`id` {{FILTER}} AND `phenotype_id` = ? %s %s GROUP BY `datasets`.`id` {{SORT_BITS}} LIMIT ?, ?";
	...
	private static final String SELECT_ALL_FOR_COMPOUND    = "SELECT `datasets`.*,  `experiments`.*, `experimenttypes`.*, `locations`.*, `countries`.*, `licenses`.*, COUNT(DISTINCT `germinatebase_id`) AS nr_of_data_objects, COUNT(`compound_value`) AS nr_of_data_points FROM " + DATA_POINTS_SUB_QUERY + " LEFT JOIN `compounddata` ON `compounddata`.`dataset_id` = `datasets`.`id` {{FILTER}} AND `compound_id` = ? %s %s GROUP BY `datasets`.`id` {{SORT_BITS}} LIMIT ?, ?";
	...
	/**
	 * Returns the paginated {@link Dataset}s matching the filter and the phenotype id
	 *
	 * @param user        The user requesting the data
	 ...
	 */
	public static PaginatedServerResult<List<Dataset>> getAllForFilterAndTrait(UserAuth user, PartialSearchQuery filter, ExperimentType type, Long phenotypeId, Pagination pagination) throws DatabaseException, InvalidSearchQueryException, InvalidArgumentException, InvalidColumnException, InsufficientPermissionsException
	{
		...
		{
			case trials:
				formatted = SELECT_ALL_FOR_TRAIT.replace("{{SORT_BITS}}", pagination.getSortQuery());
				break;
			case compound:
				formatted = SELECT_ALL_FOR_COMPOUND.replace("{{SORT_BITS}}", pagination.getSortQuery());
				break;
			default:
				formatted = null;
		}
