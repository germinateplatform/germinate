...
{
	public static final String[] COLUMNS_TABLE = {Dataset.ID, "experimenttypes.description", "experiment_name", Dataset.DESCRIPTION, Dataset.CONTACT, Dataset.DATE_START, Dataset.DATE_END, Dataset.NR_OF_DATA_OBJECTS, Dataset.NR_OF_DATA_POINTS};
	...
	public static ServerResult<List<Dataset>> getByIds(UserAuth user, List<Long> ids) throws DatabaseException, InsufficientPermissionsException
	{
		...
		try
		{
			...
			return getDatabaseObjectQuery(formatted, user, null, null, null)
					.setLongs(ids)
					.setInt(0)
	...
	public static PaginatedServerResult<List<Dataset>> getByIdsPaginated(UserAuth user, List<Long> ids, Pagination pagination) throws DatabaseException, InsufficientPermissionsException
	{
		...
		try
		{
			...
			return getDatabaseObjectQuery(formatted, user, null, null, pagination.getResultSize())
					.setLongs(ids)
	...
	private static GerminateTableQuery getGerminateTableQuery(String query, UserAuth user, PartialSearchQuery filter, ExperimentType type, String[] columnNames) throws DatabaseException, InvalidArgumentException, InsufficientPermissionsException, InvalidSearchQueryException, InvalidColumnException
	{
		...
		GerminateTableQuery result = getFilteredGerminateTableQuery(filter, formatted, COLUMNS_TABLE, columnNames);
	...
	private static String getFormattedQuery(String query, boolean isPrivate, GatekeeperUserWithPassword details, ExperimentType type) throws InsufficientPermissionsException
	{
		/*
		...
		String experimentTypeExtras = type != null ? " AND experimenttypes.description = '" + type.name() + "'" : "";
	...
	{
		ServerResult<List<Dataset>> availableDatasets = DatasetManager.getForUser(userAuth);
	...
	public static ServerResult<List<Dataset>> getForUser(UserAuth userAuth) throws DatabaseException
	{
		...
		if (isPrivate)
		{
			...
			if (details.isAdmin())
			{
				return new DatabaseObjectQuery<Dataset>(SELECT_FOR_USER_ADMIN, userAuth)
						.run()
						.getObjects(Dataset.Parser.Inst.get());
			}
		...
		else
		{
			...
			return new DatabaseObjectQuery<Dataset>(SELECT_FOR_USER_PUBLIC, userAuth)
					.run()
					.getObjects(Dataset.Parser.Inst.get());
		}
	}
	...
	/**
	 * Returns <code>true</code> if at least one of the {@link Dataset}s with the given ids has a source file, i.e. {@link Dataset#SOURCE_FILE} isn't
	 * empty.
	 *
	 * @param userAuth   The user requesting the data
	 * @param datasetIds The ids of the datasets
	 * @return <code>true</code> if at least one of the {@link Dataset}s with the given ids has a source file, i.e. {@link Dataset#SOURCE_FILE} isn't
	 * empty.
	 * @throws DatabaseException                Thrown if the communication with the database fails
	 * @throws InsufficientPermissionsException Thrown if the use doesn't have sufficient permissions to access the data
	 */
	public static ServerResult<Boolean> hasSourceFile(UserAuth userAuth, List<Long> datasetIds) throws DatabaseException, InsufficientPermissionsException
	{
		ServerResult<List<Dataset>> datasets = getByIds(userAuth, datasetIds);

		boolean hasDataFile = false;

		for (Dataset dataset : datasets.getServerResult())
		{
		...
		return new ServerResult<>(datasets.getDebugInfo(), hasDataFile);
	}
}
