...
public class DatasetManager extends AbstractManager<Dataset>
{
	...
	private static final String SELECT_ALL_FOR_TRAIT       = "SELECT `datasets`.*,  `experiments`.*, `experimenttypes`.*, `locations`.*, `countries`.*, `licenses`.*, COUNT(DISTINCT `germinatebase_id`) AS nr_of_data_objects, COUNT(`phenotype_value`) AS nr_of_data_points FROM " + DATA_POINTS_SUB_QUERY + " LEFT JOIN `phenotypedata` ON `phenotypedata`.`dataset_id` = `datasets`.`id` {{FILTER}} %s %s AND `phenotype_id` = ? GROUP BY `datasets`.`id` {{SORT_BITS}} LIMIT ?, ?";
	...
	private static final String SELECT_ALL_FOR_COMPOUND    = "SELECT `datasets`.*,  `experiments`.*, `experimenttypes`.*, `locations`.*, `countries`.*, `licenses`.*, COUNT(DISTINCT `germinatebase_id`) AS nr_of_data_objects, COUNT(`compound_value`) AS nr_of_data_points FROM " + DATA_POINTS_SUB_QUERY + " LEFT JOIN `compounddata` ON `compounddata`.`dataset_id` = `datasets`.`id` {{FILTER}} %s %s AND `compound_id` = ? GROUP BY `datasets`.`id` {{SORT_BITS}} LIMIT ?, ?";
