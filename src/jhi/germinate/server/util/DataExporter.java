...
{
	private static final String QUERY_EXPORT_ROWS_GENOTYPE_INTERNAL   = "SELECT DISTINCT germinatebase.name FROM germinatebase LEFT JOIN groupmembers ON groupmembers.foreign_id = germinatebase.id LEFT JOIN groups ON groups.id = groupmembers.group_id LEFT JOIN grouptypes ON grouptypes.id = groups.grouptype_id LEFT JOIN genotypes ON genotypes.germinatebase_id = germinatebase.id LEFT JOIN datasets ON datasets.id = genotypes.dataset_id LEFT JOIN experiments ON experiments.id = datasets.experiment_id LEFT JOIN experimenttypes ON experimenttypes.id = experiments.experiment_type_id WHERE groups.id IN (%s) AND dataset_id IN (%s)";
