...
\begin{description}
	  ...
	  \item[ADD] $<$servlet$>$ and $<$servlet-mapping$>$ entries in web.xml are no longer required.
	  \item[CHG] More template style changes.
	  \item[CHG] Restricted decimal places of numbers to 2 digits where sensible.
	  \item[CHG] Changed share-widget to mirror look-and-feel of new language selector.
	  \item[ADD] Introduced new ShareWidget.java that replaces ShareButtonBar.java.
	  \item[FIX] Fixed multiple CSS issues.
	  \item[FIX] Dataset overview table showing the wrong column content.
	  \item[ADD] Germinate now uses HTML5 local storage instead of cookies (some cookies remain).
	  \item[ADD] Added pedigree database tables.
	  \item[CHG] Updated jQuery to 2.1.4.
	  \item[CHG] Updated jQueryUI to 1.11.4.
	  \item[ADD] Added new properties entry: \texttt{Germinate.HideIdColumns} that controls if internal id columns are shown in tables.
	  \item[CHG] Updated httpclient and httpcore to newer versions.
	  \item[CHG] Major rewrite of PaginationTable.
	  \item[ADD] Added initial functionality to handle marking of not only accessions.
	  \item[CHG] Changed RPC signatures to use Pagination.java instead of a variable each for start, length and sorting, etc.
	  \item[CHG] Updated FontAwesome to 4.4.0.
	  \item[CHG] Added more database tables to conform with the MCPD.
	  \item[CHG] Removed no longer required columns from "germinatebase".
	  \item[CHG] Moved "flowering\textunderscore season\textunderscore start" and "growing\textunderscore season\textunderscore start" to "attributes".
	  \item[ADD] Markers and collecting sites can now be marked for the "shopping cart" as well as accessions.
	  \item[FIX] Fixed issue with floating table headers not loading.
	  \item[CHG] Replaced some raster image resources with vector image resources.
	  \item[CHG] logo.png and crop.png now have to be .svg files.
	  \item[FIX] Fixed issue where Germinate would not undeploy from Tomcat from time to time.
	  \item[ADD] Added pagination to dataset tables.
	  \item[ADD] Introduced caching of database query results to increase performance and decrease the load on the database while still returning up-to-date data.
	  \item[CHG] Removed restrictions for search terms on the groups item filters. Search terms can now include SQL special characters as well.
	  \item[FIX] Fixed issue with the allele frequency data export. Items were exported independent of the dataset that was selected.
	  \item[DEL] Removed table 'groupdatasets' as no longer required.
	  \item[CHG] Rewrote SQL queries formerly using 'groupdatasets' table.
	  \item[CHG] Restyling of d3.js chart options panel.
	  \item[CHG] Changed package structure to reflect new internal guideline.
	  \item[FIX] Fixed an issue with session persistence on the server.
	  \item[ADD] Added "number of data points" to the datasets table.
	  \item[ADD] Added "search" menu item to main menu. This can be disabled/enabled with a config.properties entry ("Germinate.Template.Show.Search").
	  \item[ADD] Added initial table filtering feature.
	  \item[CHG] Changed PassportPage to use the new Accession data object.
	  \item[CHG] Added pagination to the attributes table on the passport page.
	  \item[CHG] Added option to show all locations on the location overview page.
	  \item[ADD] Added "read-only" mode where the user is not allowed to add/delete/update the data in the database. This can be disabled/enabled with a config.properties entry ("Germinate.IsReadOnly").
	  \item[ADD] Added tooltps to shopping cart indicator to explain which number represents which type of marked item.
	  \item[FIX] Fixed an issue where the context menu on markable item tables didn't show up when the actual marker checkbox was right-clicked.
	  ...
	  \item[FIX] Fixed issue with duplicate gradient widgets on locations overview page.
	  \item[ADD] Added administrator configuration page (\#admin-config).
	  \item[CHG] Complete rewrite of the main navigation menu to allow for sub-menus.
	  \item[ADD] Added option to download data for marked accessions from accessions overview page.
	  \item[FIX] Fixed loading indicator forcing scrollbars on tables when using Firefox.
	  \item[ADD] Added column filtering to Dataset tables.
	  \item[ADD] Added the option to download phenotypic data from the phenotypic dataset overview page (download icon per row).
	  \item[ADD] Added table options panel that allows showing/hiding the table filter headers.
	  \item[CHG] Rewrote table options panels for a more consistant look.
	  \item[FIX] Fixed "Continue" button being visible on genotype-datasets page even if genotype-export isn't available
	  \item[CHG] Improved performance of stored procedure to export germinatebase table for marked accessions.
	  \item[CHG] Right-aligned number columns.
	  \item[ADD] Added "mark all items" to selected tables. This will mark all the items from all pages of the paginated table not just the current page.
	  \item[FIX] Fixed issue with Internet Explorer where it wouldn't accept a specific (valid) jQuery call to open/close the table filter panel.
	  \item[FIX] Fixed file encoding issues when writing climate chart data files.
	  \item[CHG] Better integration of Flapjack by directly calling Flapjack's API for file manipulation instead of running the .jar file externally.
	  \item[ADD] Added "Color by dataset" to phenotype by phenotype charts.
	  \item[ADD] Added collecting site search based on user-defined polygons.
	  \item[ADD] Added units to Climate dropdown box
	  \item[FIX] Fixed issue with wrong items being selected from phenotype by phenotype chart.
...
\subsection{Migration from 3.2.0d to 3.3.0}
\begin{itemize}
	\item Install Java 8 on your server running Tomcat.
	\item Delete the "Flapjack" folder in your \path{instance-stuff/<your-project>/apps} folder.
